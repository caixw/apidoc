...
func TestHandler(t *testing.T) {
	...
	h.Error(NewSyntaxError(Location{URI: "erro.go"}, "", locale.ErrRequired))
	h.Warning(NewSyntaxError(Location{URI: "warn.go"}, "", locale.ErrRequired))
	...
	a.Panic(func() { // 已经关闭 messages
		h.Error(NewSyntaxError(Location{URI: "erro"}, "", locale.ErrRequired))
	})
}
